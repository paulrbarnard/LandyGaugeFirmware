...
static int QSPI_Init(void) {
  ...
  ESP_LOGI(TAG_LCD, "SPI initialization succeeded");
  ...
  ESP_LOGI(TAG_LCD, "LCD communication parameters set successfully -- SPI");

  ESP_LOGI(TAG_LCD, "Install LCD driver of ST77916");
  ...
  // Read LCD ID to verify communication
  esp_err_t ret;
  int lcd_cmd = 0x04;
...
void ST77916_Init(void) {
  ST77916_Reset();
  if (!QSPI_Init()) {
    ESP_LOGE(TAG_LCD, "ST77916 failed to initialize");
  }
}
...
void Backlight_Init(void) {
  ESP_LOGI(TAG_LCD, "Initialize LCD backlight");
  ...
  ledc_channel_config_t ledc_channel = {
    ...
    .duty = 0,
    .gpio_num = EXAMPLE_LCD_PIN_NUM_BK_LIGHT,
    .speed_mode = LEDC_LS_MODE,
