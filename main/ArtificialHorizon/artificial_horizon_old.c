...
// LVGL objects
static lv_obj_t *horz_face = NULL;
...
void artificial_horizon_init(void)
{
    ...
    // Create main container with dark grey background (full screen fill)
      ...
      if (!horz_face)
    {
        horz_face = lv_obj_create(lv_scr_act());

        // After creating clock_face:
        lv_obj_set_size(horz_face, DISP_W, DISP_H);
        lv_obj_center(horz_face);

        // Don’t let layout/children expand it
        lv_obj_clear_flag(horz_face, LV_OBJ_FLAG_SCROLLABLE);
        lv_obj_clear_flag(horz_face, LV_OBJ_FLAG_OVERFLOW_VISIBLE); // clip to 360×360

        lv_obj_set_style_bg_color(horz_face, COLOR_BACKGROUND, 0);
        lv_obj_set_style_bg_opa(horz_face, LV_OPA_100, 0);
        lv_obj_set_style_border_width(horz_face, 1, 0);
        lv_obj_set_style_border_color(horz_face, COLOR_BACKGROUND, 0); //
        // lv_obj_set_style_radius(horz_face, LV_RADIUS_CIRCLE, 0);
        lv_obj_move_background(horz_face);
        ESP_LOGI(TAG, "horz_face created and centered");
    }

      
    // Create sky/ground object
    sky_obj = lv_obj_create(horz_face);
    ...
    pitch_lines_obj = lv_obj_create(horz_face);
    ...
    roll_indicator_obj = lv_obj_create(horz_face);
    ...
    center_marker_obj = lv_obj_create(horz_face);
    ...
    lv_obj_t *mask_ring = lv_obj_create(horz_face);
    lv_obj_set_size(mask_ring, DISP_W, DISP_H);
    ...
    lv_obj_t *shadow_dark = lv_obj_create(horz_face);
    ...
    shadow_light = lv_obj_create(horz_face);
...
void artificial_horizon_set_visible(bool visible)
{
    if (horz_face) {
        if (visible) {
            lv_obj_clear_flag(horz_face, LV_OBJ_FLAG_HIDDEN);
        } else {
            lv_obj_add_flag(horz_face, LV_OBJ_FLAG_HIDDEN);
        }
    }
}
